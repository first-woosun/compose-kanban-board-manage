...
- [ ] Task 상태 별 담당자 지정 규칙
    ...
    - [ ] Review는 담당자가 지정되지 않으면 Task를 생성할 수 없다.
    - [ ] Done은 담당자가 지정되지 않으면 Task를 생성할 수 없다.
- [ ] Task 생성 규칙
    - [ ] To Do 태스크는 담당자가 지정되지 않아도 생성할 수 있다.
    - [ ] In Progress 태스크를 생성하려면 담당자가 지정되어야 한다.
    - [ ] Review 태스크를 생성하려면 담당자가 지정되어야 한다.
    - [ ] Done 태스크를 생성하려면 담당자가 지정되어야 한다.

## UI 기능 구현 목록

- [ ] TaskDialog
    - [ ] 수행할 작업에 따라 Dialog의 구성이 달라진다.
        - [ ] 태스크를 생성할 때는 Title에 “새 태스크 생성”텍스트가 표시되고 취소, 생성 버튼이 표시된다.
        - [ ] 태스크를 수정할 때는 Title에 “기존 태스크 수정”텍스트가 표시되고 취소, 삭제, 수정 버튼이 표시된다.
        - [ ] 수정할 태스크가 To Do 상태인 경우 담당자 선택 영역에 없음 버튼이 표시된다.
        - [ ] 수정할 태스크가 To Do 상태가 아닌 경우 담당자 선택 영역에 없음 버튼이 표시되지 않는다.
- [ ] SnackBar
    ...
    - [ ] 불가능한 상태 변경을 시도하면 "해당 상태로 옮길 수 없다." SnackBar가 표시되어야 한다.
